...
impl Domain for Pacman {
    ...
    fn command(&self) -> ClapCommand {
        PkgArgs::augment_args(ClapCommand::new("pkg").about("Manage packages (Pacman)"))
    }
...
impl PackageManager for Pacman {
    ...
    fn install(&self, packages: &[String]) -> Result<Box<dyn ExecutableCommand>> {
        Ok(Box::new(SystemCommand::new("pacman").arg("-S").arg("--noconfirm").arg("--").args(packages)))
    }

    fn remove(&self, packages: &[String], _purge: bool) -> Result<Box<dyn ExecutableCommand>> {
        Ok(Box::new(SystemCommand::new("pacman").arg("-Rs").arg("--noconfirm").arg("--").args(packages)))
    }

    fn search(&self, query: &str) -> Result<Box<dyn ExecutableCommand>> {
        Ok(Box::new(SystemCommand::new("pacman").arg("-Ss").arg("--").arg(query)))
    }
