...
use crate::os::{MonitorManager, ExecutableCommand, Domain};
...
impl Domain for StandardMonitor {
    ...
    fn execute(&self, _matches: &ArgMatches, _app: &ClapCommand) -> Result<Box<dyn ExecutableCommand>> {
        self.live_stats()
    }
}
...
impl ExecutableCommand for LiveStatsCommand {
    fn execute(&self) -> Result<()> {
        ...
        for (name, data) in &networks {
            println!("{}: RX {} bytes, TX {} bytes", name, data.total_received(), data.total_transmitted());
        }
        let disks = Disks::new_with_refreshed_list();
        for disk in &disks {
            println!("{:?}: {} / {} bytes", disk.name(), disk.total_space() - disk.available_space(), disk.total_space());
        }
        Ok(())
    }
