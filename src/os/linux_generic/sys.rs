...
use crate::os::{SysManager, ExecutableCommand, Domain, SysInfoData, SysTimeData, OutputFormat};
use crate::cli::{SysArgs, SysAction};
...
impl Domain for StandardSys {
    ...
    fn command(&self) -> ClapCommand {
        SysArgs::augment_args(ClapCommand::new("sys").about("Manage core system (updates, power, time)"))
    }
    fn execute(&self, matches: &ArgMatches, _app: &ClapCommand) -> Result<Box<dyn ExecutableCommand>> {
        ...
        match &args.action {
            ...
            SysAction::Time { action, value, format } => self.time(action, value.as_deref(), *format),
        }
    }
    fn complete(&self, _line: &str, words: &[&str], last_word_complete: bool) -> Result<Vec<String>> {
        if is_completing_arg(words, &["ao", "sys", "power"], 1, last_word_complete) {
            return Ok(vec!["reboot".to_string(), "shutdown".to_string(), "suspend".to_string(), "hibernate".to_string()]);
        }
        if is_completing_arg(words, &["ao", "sys", "time"], 1, last_word_complete) {
            return Ok(vec!["status".to_string(), "set".to_string(), "sync".to_string()]);
        }
        Ok(vec![])
    }
}
...
impl SysManager for StandardSys {
    ...
    fn power(&self, state: &str, now: bool, force: bool) -> Result<Box<dyn ExecutableCommand>> {
        ...
        if now { cmd = cmd.arg("--now"); }
        if force { cmd = cmd.arg("--force"); }
        Ok(Box::new(cmd))
    }
    ...
    fn time(&self, action: &str, value: Option<&str>, format: OutputFormat) -> Result<Box<dyn ExecutableCommand>> {
        ...
        match action {
            "set" => {
                if let Some(v) = value {
                    cmd = cmd.arg("set-timezone").arg(v);
                } else {
...
impl ExecutableCommand for SysInfoCommand {
    fn execute(&self) -> Result<()> {
        ...
        match self.format {
            OutputFormat::Table => {
                ...
                table.add_row(vec!["Total Memory", &format!("{} bytes", data.total_memory)]);
    ...
    fn dry_run(&self) -> Result<()> { println!("[DRY RUN] Sys info (format: {:?})", self.format); Ok(()) }
    fn print(&self) -> Result<()> { println!("sys info --format {:?}", self.format); Ok(()) }
    fn as_string(&self) -> String { format!("sys info --format {:?}", self.format) }
...
impl ExecutableCommand for SysTimeCommand {
    fn execute(&self) -> Result<()> {
        ...
        for line in stdout.lines() {
            let line = line.trim();
            if let Some(val) = line.strip_prefix("Local time:") { data.local_time = val.trim().to_string(); }
            else if let Some(val) = line.strip_prefix("Universal time:") { data.universal_time = val.trim().to_string(); }
            else if let Some(val) = line.strip_prefix("RTC time:") { data.rtc_time = val.trim().to_string(); }
            else if let Some(val) = line.strip_prefix("Time zone:") { data.time_zone = val.trim().to_string(); }
            else if let Some(val) = line.strip_prefix("System clock synchronized:") { data.system_clock_synchronized = val.trim().to_string(); }
            else if let Some(val) = line.strip_prefix("NTP service:") { data.ntp_service = val.trim().to_string(); }
            else if let Some(val) = line.strip_prefix("RTC in local TZ:") { data.rtc_in_local_tz = val.trim().to_string(); }
        }
    ...
    fn dry_run(&self) -> Result<()> { println!("[DRY RUN] timedatectl status (format: {:?})", self.format); Ok(()) }
    fn print(&self) -> Result<()> { println!("timedatectl status (format: {:?})", self.format); Ok(()) }
    fn as_string(&self) -> String { format!("timedatectl status --format {:?}", self.format) }
    fn is_structured(&self) -> bool {
        matches!(self.format, OutputFormat::Json | OutputFormat::Yaml | OutputFormat::Original)
    }
}
