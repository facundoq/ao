...
use crate::os::{UserManager, ExecutableCommand, Domain, UserInfo, OutputFormat};
use crate::cli::{UserArgs, UserAction};
...
impl Domain for StandardUser {
    ...
    fn execute(&self, matches: &ArgMatches, _app: &ClapCommand) -> Result<Box<dyn ExecutableCommand>> {
        ...
        match &args.action {
            UserAction::List { all, groups, format } => self.list(*all, *groups, *format),
            UserAction::Add { username, groups, shell, system } => self.add(username, groups.as_deref(), shell.as_deref(), *system),
            UserAction::Del { username, purge } => self.del(username, *purge),
            UserAction::Mod { username, action, value } => self.mod_user(username, action, value),
    ...
    fn complete(&self, _line: &str, words: &[&str], last_word_complete: bool) -> Result<Vec<String>> {
        ...
        if is_completing_arg(words, &["ao", "user", "mod"], 2, last_word_complete) {
            return Ok(vec!["add-group".to_string(), "del-group".to_string(), "shell".to_string(), "home".to_string()]);
        }
        ...
        if is_completing_arg(words, &["ao", "user", "mod"], 3, last_word_complete) {
            ...
            if action == "add-group" || action == "del-group" {
                let output = Command::new("cut").arg("-d:").arg("-f1").arg("/etc/group").output()?;
...
impl UserManager for StandardUser {
    fn list(&self, all: bool, groups: bool, format: OutputFormat) -> Result<Box<dyn ExecutableCommand>> {
        ...
        Ok(Box::new(UserListCommand { all, groups, format }))
    }

    fn add(&self, username: &str, groups: Option<&str>, shell: Option<&str>, system: bool) -> Result<Box<dyn ExecutableCommand>> {
    ...
    fn mod_user(&self, username: &str, action: &str, value: &str) -> Result<Box<dyn ExecutableCommand>> {
    ...
    fn passwd(&self, username: &str) -> Result<Box<dyn ExecutableCommand>> {
        Ok(Box::new(PasswdCommand { username: username.to_string() }))
    }

    fn get_users(&self) -> Result<Vec<String>> {
        let output = Command::new("cut").arg("-d:").arg("-f1").arg("/etc/passwd").output()?;
        let stdout = String::from_utf8_lossy(&output.stdout);
...
pub struct UserListCommand { pub all: bool, pub groups: bool, pub format: OutputFormat }
    ...
    fn execute(&self) -> Result<()> {
        ...
        for line in stdout.lines() {
            ...
            if parts.len() >= 7 {
                ...
                if !self.all && uid < 1000 && uid != 0 { continue; }
        ...
        match self.format {
            ...
            OutputFormat::Yaml => { println!("{}", serde_yaml::to_string(&users)?); }
            OutputFormat::Original => unreachable!(),
        }
        Ok(())
    }
    fn dry_run(&self) -> Result<()> { println!("[DRY RUN] List users (format: {:?})", self.format); Ok(()) }
    fn print(&self) -> Result<()> { println!("list users (format: {:?})", self.format); Ok(()) }
    fn as_string(&self) -> String { format!("list users --format {:?}", self.format) }
...
impl ExecutableCommand for UserAddCommand {
    fn execute(&self) -> Result<()> {
        ...
        if self.system { cmd = cmd.arg("--system"); }
        if let Some(ref s) = self.shell { cmd = cmd.arg("--shell").arg(s); }
        if let Some(ref g) = self.groups { cmd = cmd.arg("--groups").arg(g); }
    ...
    fn as_string(&self) -> String {
        ...
        if self.system { s.push_str(" --system"); }
        if let Some(ref sh) = self.shell { s.push_str(&format!(" --shell {}", sh)); }
        if let Some(ref g) = self.groups { s.push_str(&format!(" --groups {}", g)); }
...
impl ExecutableCommand for UserDelCommand {
    fn execute(&self) -> Result<()> {
        ...
        if self.purge { cmd = cmd.arg("-r"); }
        cmd.arg("--").arg(&self.username).execute()
    }
    fn dry_run(&self) -> Result<()> { println!("[DRY RUN] {}", self.as_string()); Ok(()) }
    fn print(&self) -> Result<()> { println!("{}", self.as_string()); Ok(()) }
    fn as_string(&self) -> String {
        if self.purge { format!("userdel -r -- {}", self.username) }
        else { format!("userdel -- {}", self.username) }
    }
}
...
impl ExecutableCommand for UserModCommand {
    fn execute(&self) -> Result<()> {
        match self.action.as_str() {
            "add-group" => SystemCommand::new("usermod").arg("-aG").arg(&self.value).arg("--").arg(&self.username).execute(),
            "del-group" => SystemCommand::new("gpasswd").arg("-d").arg(&self.username).arg("--").arg(&self.value).execute(),
            "shell" => SystemCommand::new("usermod").arg("-s").arg(&self.value).arg("--").arg(&self.username).execute(),
            "home" => SystemCommand::new("usermod").arg("-d").arg(&self.value).arg("-m").arg("--").arg(&self.username).execute(),
