...
use sysinfo::{CpuRefreshKind, Disks, MemoryRefreshKind, Networks, Process, RefreshKind, System, Users};
use crate::dashboard::rss::get_rss;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ProcessSort { Pid, Cpu, Mem, Name, User }
...
impl<'a> App<'a> {
    pub fn new(detected_system: &'a DetectedSystem) -> Self {
        ...
        let mut app = Self {
            ...
            detected_system,
            tab_index: 0,
            tabs: vec!["Overview", "Storage", "Process", "User", "Network", "Service", "Virtualization", "Sensors", "Charts"],
            selected_index: 0,
            sessions: Vec::new(),
            users: Vec::new(),
            interfaces: Vec::new(),
            ...
            containers: Vec::new(),
            sensors: Vec::new(),
            ram_config: "Unknown".to_string(),
            ram_speed: "Unknown".to_string(),
    ...
    pub fn on_tick(&mut self) {
        ...
        let mem_percent = if mem_total > 0.0 { (self.system_info.used_memory() as f64 / mem_total) * 100.0 } else { 0.0 };
        self.mem_history.push((now_sec, mem_percent));
        if self.mem_history.len() > 60 { self.mem_history.remove(0); }
        let swap_total = self.system_info.total_swap() as f64;
        let swap_percent = if swap_total > 0.0 { (self.system_info.used_swap() as f64 / swap_total) * 100.0 } else { 0.0 };
        ...
        if let Ok(s) = self.detected_system.overview.get_sensors() {
            for sensor in &s {
                let entry = self.max_temps.entry(sensor.label.clone()).or_insert(sensor.temperature);
                if sensor.temperature > *entry { *entry = sensor.temperature; }
            }
            self.sensors = s;
        }
        match self.tab_index {
            3 => { if let Ok(mut s) = self.detected_system.user.get_sessions(None, true, Some(50)) { s.reverse(); self.sessions = s; } }
            4 => { if let Ok(i) = self.detected_system.net.get_interfaces() { self.interfaces = i; } }
            5 => { if let Ok(s) = self.detected_system.svc.get_all_services_info() { self.services = s; } }
            6 => { if let Ok(c) = self.detected_system.virt.get_containers() { self.containers = c; } }
            _ => {}
        }
    }
    ...
    pub fn refresh_process_data(&mut self) {
        ...
        self.top_cpu_processes = self.calculate_top_cpu_processes(10);
        self.top_mem_processes = self.calculate_top_mem_processes(10);
        self.sorted_processes = self.calculate_sorted_processes();
        self.flattened_tree = self.calculate_flattened_tree();
    }

    fn calculate_sorted_processes(&self) -> Vec<ProcessSummary> {
        let current_uid = unsafe { libc::getuid() };
        let mut processes: Vec<&Process> = self.system_info.processes().values()
            .filter(|p| {
                if self.show_only_current_user {
                    if let Some(uid) = p.user_id() {
                        let uid_u32: u32 = uid.to_string().parse().unwrap_or(u32::MAX);
                        if uid_u32 != current_uid { return false; }
                    } else { return false; }
                }
        ...
        processes.sort_by(|a, b| {
            let res = match self.process_sort {
                ...
                ProcessSort::User => {
                    ...
                    let bu = b.user_id().and_then(|id| self.users_list.get_user_by_id(id)).map(|u| u.name()).unwrap_or("");
                    au.cmp(bu)
                }
            };
            if self.sort_descending { res.reverse() } else { res }
        });
        processes.into_iter().map(|p| {
            let user_name = p.user_id().and_then(|uid| self.users_list.get_user_by_id(uid)).map(|u| u.name().to_string()).unwrap_or_else(|| "unknown".to_string());
            ProcessSummary {
                pid: p.pid(), name: p.name().to_string_lossy().to_string(), user: user_name,
                memory: get_rss(p.pid()), cpu: p.cpu_usage() / self.system_info.cpus().len() as f32,
                command: p.exe().map(|e| e.to_string_lossy().to_string()).unwrap_or_else(|| "".to_string()),
            }
        }).collect()
    }

    fn calculate_process_tree(&mut self) -> Vec<ProcessTreeNode> {
        ...
        for (pid, process) in processes {
            if self.show_only_current_user {
                if let Some(uid) = process.user_id() {
                    let uid_u32: u32 = uid.to_string().parse().unwrap_or(u32::MAX);
                    if uid_u32 != current_uid { continue; }
                } else { continue; }
            }
            if self.hide_kernel_processes {
        ...
        for (pid, process) in &filtered {
            if let Some(parent_pid) = process.parent() {
                if filtered.contains_key(&parent_pid) { tree.entry(parent_pid).or_default().push(*pid); }
                else { root_pids.push(*pid); }
            } else { root_pids.push(*pid); }
        }
        fn build_node(pid: sysinfo::Pid, depth: u32, processes: &HashMap<sysinfo::Pid, &Process>, tree: &HashMap<sysinfo::Pid, Vec<sysinfo::Pid>>, users: &Users, cpu_count: f32) -> ProcessTreeNode {
            ...
            if let Some(child_pids) = tree.get(&pid) {
                for &child_pid in child_pids {
                    let child_node = build_node(child_pid, depth + 1, processes, tree, users, cpu_count);
                    total_cpu += child_node.total_cpu;
                    total_mem += child_node.total_mem;
                    children.push(child_node);
                }
            }
            let user_name = process.user_id().and_then(|uid| users.get_user_by_id(uid)).map(|u| u.name().to_string()).unwrap_or_else(|| "unknown".to_string());
            ProcessTreeNode { pid, name: process.name().to_string_lossy().to_string(), user: user_name, cpu_usage: process.cpu_usage() / cpu_count, memory: get_rss(pid), depth, children, total_cpu, total_mem }
        }
        let mut root_nodes = Vec::new();
        for pid in root_pids { root_nodes.push(build_node(pid, 0, &filtered, &tree, &self.users_list, cpu_count)); }
        root_nodes.sort_by(|a, b| b.total_cpu.partial_cmp(&a.total_cpu).unwrap_or(std::cmp::Ordering::Equal));
        root_nodes
    }

    fn calculate_top_cpu_processes(&self, count: usize) -> Vec<ProcessSummary> {
        let mut all = Vec::new();
    ...
    fn calculate_top_mem_processes(&self, count: usize) -> Vec<ProcessSummary> {
        let mut all = Vec::new();
        fn flatten(node: &ProcessTreeNode, out: &mut Vec<ProcessSummary>) {
            out.push(ProcessSummary { pid: node.pid, name: node.name.clone(), user: node.user.clone(), memory: node.total_mem, cpu: node.total_cpu, command: "".to_string() });
            for child in &node.children { flatten(child, out); }
    ...
    fn calculate_flattened_tree(&self) -> Vec<FlattenedTreeNode> {
        ...
        fn flatten_tree(node: &ProcessTreeNode, max_depth: u32, rows: &mut Vec<FlattenedTreeNode>) {
            ...
            let cpu_str = if node.depth < max_depth && !node.children.is_empty() { format!("({:.1}%)", node.total_cpu * 100.0) } else { format!("{:.1}%", node.cpu_usage * 100.0) };
            let mem_str = if node.depth < max_depth && !node.children.is_empty() { format!("{} ({})", format_bytes(node.memory), format_bytes(node.total_mem)) } else { format_bytes(node.memory) };
            rows.push(FlattenedTreeNode { pid: node.pid.to_string(), user: node.user.clone(), cpu: cpu_str, mem: mem_str, name: format!("{}{}{}", indent, prefix, node.name), depth: node.depth });
            if node.depth + 1 < max_depth { for child in &node.children { flatten_tree(child, max_depth, rows); } }
        }
        for root in &self.process_tree { flatten_tree(root, self.tree_expansion_depth, &mut flat); }
        flat
    }
    ...
    pub fn get_current_list_len(&self) -> usize {
        match self.tab_index {
            1 => self.disks.len(),
            2 => { if self.use_tree_view { self.flattened_tree.len() } else { self.sorted_processes.len() } }
    ...
    pub fn next_tab(&mut self) { self.tab_index = (self.tab_index + 1) % self.tabs.len(); self.selected_index = 0; }
    pub fn prev_tab(&mut self) { if self.tab_index > 0 { self.tab_index -= 1; } else { self.tab_index = self.tabs.len() - 1; } self.selected_index = 0; }
    pub fn on_up(&mut self) { if self.selected_index > 0 { self.selected_index -= 1; } }
    pub fn on_down(&mut self) { let len = self.get_current_list_len(); if len > 0 && self.selected_index < len.saturating_sub(1) { self.selected_index += 1; } }
    pub fn on_page_up(&mut self) { let items_per_page = 20; if self.selected_index >= items_per_page { self.selected_index -= items_per_page; } else { self.selected_index = 0; } }
    pub fn on_page_down(&mut self) { let len = self.get_current_list_len(); if len > 0 { let items_per_page = 20; let max_idx = len.saturating_sub(1); self.selected_index = (self.selected_index + items_per_page).min(max_idx); } }
}
